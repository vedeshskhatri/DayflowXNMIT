    ...
    <>
      ...
      <div className="fixed bottom-6 right-6 z-[9999] flex flex-col gap-2 items-end pointer-events-none">
        {toasts.map((t) => (
          <div
            key={t.id}
            className="animate-points-toast flex items-center space-x-2.5 bg-white border border-navy/15 shadow-modal text-text-primary px-4 py-2.5 rounded-2xl"
          >
            <div className="w-8 h-8 rounded-xl bg-copper-muted border border-copper/30 flex items-center justify-center text-sm">
              🪙
            </div>
            <div>
      ...
      <div className="fixed bottom-24 right-6 z-[9999] flex flex-col gap-2 items-end pointer-events-none">
        {streakToasts.map((t) => (
          <div
            key={t.id}
            className="animate-points-toast flex items-center space-x-3 bg-white border-2 border-copper shadow-modal text-text-primary px-5 py-3 rounded-2xl"
          >
            <div className="w-10 h-10 rounded-xl bg-copper-muted border border-copper/40 flex items-center justify-center">
              <Flame className="w-6 h-6 text-copper fill-copper" />
            </div>
            <div>
              <div className="font-heading font-bold text-navy-dark text-base">
